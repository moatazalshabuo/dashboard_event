<template>
  <div>
    <h1 class="border-b border-purple-600 font-bold inline-block">المنظمين</h1>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-6 py-4 grid grid-flow-col col-span-2">
                  <button v-on:click="deleteOrganizer(li.id)" class="rounded border border-red-600 p-2 mx-1 text-center">
...
export default {
  ...
  methods: {
    ...
    GetOrganizes() {
      axios.get("/api/get/organize/").then((res) => {
        this.list = res.data.data;
      });
    },
    GetOrganize(id) {
      axios.get(`/api/get/organize/${id}`).then((res) => {
      ...
      axios
        .post(`/api/update/organize/${this.id_update}`, form, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
      ...
      axios.delete(`/api/delete/organizer/${id}`).then((res)=>{
